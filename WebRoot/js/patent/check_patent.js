function checkPatent(){
	...
	$('.end-button').unbind().on("click",function(){
		...
		if (data.export_id != "" && data.export_name != "") {
			location.href = "/teacherms/Admin/admin_ExportExcelCollection?tableName=TeacherPatent&export_id=" + (data.export_id).substring(0,data.export_id.length-1) + "&export_name=" + (data.export_name).substring(0,data.export_name.length-1);
		} else {
			alert("请选择数据");
		}
		data.export_id="";
