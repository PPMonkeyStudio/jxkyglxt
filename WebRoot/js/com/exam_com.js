			...
			for (var i = 0; i < xhr_data.ObjDatas.length; i++) {
				...
				for (var j = 0; j < xhr_data.ObjDatas[i].length; j++) {
					if (j == xhr_data.ObjDatas[i].length - 1) {
						str += '<td><input type="hidden" value="' + xhr_data.ObjDatas[i][j] + '" ><img class="checkimg1" src="/teacherms/img/check.png" /><img class="checkimg2" src="/teacherms/img/unlock.png" /></td>'
						continue;
					}
