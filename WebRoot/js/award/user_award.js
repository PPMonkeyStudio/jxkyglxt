	...
	$.ajax({
		...
		success : function(xhr_data) {
			...
			$('.modiButton').click(function() {
				...
				$('#award_modal .btn-danger').text("修改");
	...
	$('.export-info').unbind().click(function() {
		...
		$('.end-button').unbind().on("click", function() {
			...
			if (data.export_id != "" && data.export_name != "") {
				location.href = "/teacherms/Teacher/teacher_userExportExcelCollection?tableName=TeacherAward&export_id=" + (data.export_id).substring(0, data.export_id.length - 1) + "&export_name=" + (data.export_name).substring(0, data.export_name.length - 1);
			} else {
				alert("请选择数据");
			}
			data.export_id = "";
